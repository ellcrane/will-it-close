    ...
    for i in range(len(dataframe)):
        ...
        for j in range(len(dataframe['results'][i])):
            try:
                ...
                key_errors += 1
        try:
            summaries.append({'business_id': dataframe['yelp_business_id'][i], 'avg_'+key: (total / count)})
        except ZeroDivisionError:
            summaries.append({'business_id': dataframe['yelp_business_id'][i], 'avg_'+key: default_val})
...
if __name__ == "__main__":
    ...
    updated_google_df = pd.read_json('https://s3-us-west-2.amazonaws.com/businesspredictiondata/updated_google_data.json')

    updated_google_df['closed_on_google'] = updated_google_df['results'].apply(closed_on_google)

    restaurants_with_google_data = previously_open_US_restaurants.merge(updated_google_df, how='inner', left_on='business_id', right_on='yelp_business_id')
    ...
    google_nearby_df = pd.read_json('https://s3-us-west-2.amazonaws.com/businesspredictiondata/google_nearby.json')
    ...
    nearby_prices_rating_num = nearby_prices_and_rating.merge(google_nearby_df, how='outer', left_on='business_id', right_on='yelp_business_id')
    ...
    reviews_df = pd.read_json('https://s3-us-west-2.amazonaws.com/businesspredictiondata/small_review_df.json')

    restaurants_with_stars = restaurants_with_nearby_data.merge(reviews_df,how='left',on='business_id')

    restaurants_with_stars['one_star_text'] = restaurants_with_stars['one_star_text'].apply(str)
    restaurants_with_stars['two_to_four_star_text'] = restaurants_with_stars['two_to_four_star_text'].apply(str)
    ...
    feature_matrix = tfidf_two_to_four_open.transform(restaurants_with_stars['two_to_four_star_text'])
    ...
    unique_two_to_four_star_df = concat_unique_columns(tfidf_two_to_four_closed_df, tfidf_two_to_four_open_df, 'two_to_four_star: ')
    unique_five_star_df = concat_unique_columns(tfidf_five_closed_df, tfidf_five_open_df, 'five_star: ')
